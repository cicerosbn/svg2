...
const medidasPorTipo = {
  "Padrão": {
    dimensoes: "4,5m x 2,3m"
  },
  "Grande": {
    dimensoes: "5,0m x 2,5m"
  },
  "Paralela": {
    dimensoes: "5,3m x 2,3m"
  },
  "PCD": {
    "15,50": "5,0m x 3,1m",
    "18,50": "5,0m x 3,7m"
...
torre.addEventListener("change", esconderLocalizacao);
apto.addEventListener("change", esconderLocalizacao);
...
// SVG
function mostrarLocalizacaoVaga(numeroVaga, torre) {
  const svg = document.getElementById("svgMapa");

  // Mostra o card sempre que essa função for chamada
  cardLocalizacao.style.display = "block";

  // Segurança: SVG não existe
  if (!svg) {
    feedbackMapaErro.style.display = "flex";
    return;
  }

  // Limpa destaques anteriores
  svg.querySelectorAll("[id^='vaga-']").forEach(el => {
    el.style.fill = "";
    el.style.opacity = "";
  });

  const elementoVaga = svg.querySelector(`#vaga${numeroVaga}`);

  // ❌ Vaga NÃO encontrada no SVG
  if (!elementoVaga) {
    feedbackMapaErro.style.display = "flex";
    return;
  }
...
form.addEventListener("submit", e => {
  ...
  if (vaga) {
    ...
    if (tipo === "PCD") {
      ...
      dimensoes = medidasPorTipo[tipo].dimensoes;
    }
    ...
    resultado.innerHTML = `
      <div class="feedback sucesso">
        ...
        <span>
          ...
          possui a vaga de garagem nº <strong>${vaga.vaga}</strong>🚗<br>
